...
def plot(filename):
    ...
    v = np.linspace(-1.6, 0.2, 10, endpoint=True)
    xMin = 0.0
    xMax = 0.2
    yMin = 0.0
    yMax = 0.2
    matplotlib.rcParams['contour.negative_linestyle'] = 'solid'
    CS = plt.tricontourf(x, y, tri, Cp,levels = v,extend = 'both', cmap='gist_yarg', alpha=0.7) #CS = plt.tricontourf(x, y, tri, ux, cmap=cm.coolwarm)
    cs = plt.tricontour(np.ma.masked_outside(x,xMin,xMax), np.ma.masked_outside(y,yMin,yMax), tri, Cp, v, linewidths = 1, colors = 'black') #plt.tricontour(x, y, tri, ux, cmap=cm.coolwarm)
    ...
    savefig('Cp_rightSide.png', dpi=900, bbox_inches='tight')
    savefig('Cp_rightSide.pdf', bbox_inches='tight')
