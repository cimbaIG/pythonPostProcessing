...
def plot(filename):
    ...
    v = np.linspace(-1.9, -0.1, 10, endpoint=True)
    xMin = 0.0
    xMax = 0.1
    yMin = 0.0
    yMax = 0.2
    matplotlib.rcParams['contour.negative_linestyle'] = 'solid'
    CS = plt.tricontourf(z, x, tri, Cp,levels = v,extend = 'both', cmap='gist_yarg', alpha=0.7) #CS = plt.tricontourf(x, y, tri, ux, cmap=cm.coolwarm)
    cs = plt.tricontour(np.ma.masked_outside(z,xMin,xMax), np.ma.masked_outside(x,yMin,yMax), tri, Cp, v, linewidths = 1, colors = 'black') #plt.tricontour(x, y, tri, ux, cmap=cm.coolwarm)
    #manual_locations = [(0.025, 0.1), (0.075, 0.03), (0.075, 0.075)]
    #clabels = plt.clabel(cs, v, fmt='%4.3f', inline_spacing=-20)#, manual=manual_locations)
    #[txt.set_bbox(dict(facecolor='white', edgecolor='none', pad=0)) for txt in clabels]
    #cbar = plt.colorbar(CS, ticks=[pMin, pMax], orientation='horizontal')
    cbar = plt.colorbar(CS, ticks = v, orientation = 'horizontal', format='%.1f')
    ...
    savefig('Cp_topSide.png', dpi=900, bbox_inches='tight')
    savefig('Cp_topSide.pdf', bbox_inches='tight')
